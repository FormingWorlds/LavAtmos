...
class melt_vapor_system:
    ...
    def __init__(self, paths=None):
        ...
        print('starting guess for fO2 for the corresponding temperature:', self.fO2_interp_func)

        
        # Importing stoichiometries for reactions
        fname_cdef_values = 'cdef_values_vapor_reactions_2.csv'
        dname_cdef_values = '/data3/leoni/LavAtmos/ThermoEngine/LavAtmos/data/'
        self.cdef = pd.read_csv(dname_cdef_values+fname_cdef_values).set_index('vapor')

        fname_ml_values = 'mass_law_values.csv'
        dname_ml_values = '/data3/leoni/LavAtmos/ThermoEngine/LavAtmos/data/'
    ...
    def vaporise(self, T, P_volatile, melt_comp, volatile_comp, P_melt = 0.01,\
        ...
        with tqdm(total=len(T), file=sys.stdout) as pbar: 
            for i,t in enumerate(T):
                ...
                #residuals=[self.mass_balance_equation_fastchem(fO2,[t], volatile_comp) for fO2 in fO2_tries]
                #for i in range(len(fO2_tries) - 1):
                   # if residuals[i] * residuals[i+1] < 0:
                       # fO2_low, fO2_high = fO2_tries[i], fO2_tries[i+1]
                       # break
                
                #fO2_best = brentq(self.mass_balance_equation_fastchem,fO2_low, fO2_high, args=([t],volatile_comp))

                #print(fO2_best)

                fO2_tries = 10**self.fO2_interp_func(t)*np.array([1e-7,1e-5,1e-3,1e-2,1e-1,1e0,1e1,1e2,1e4,1e6])
                print('array of different fo2 value sto try for this run:',fO2_tries)
                ...
                vapor_partial_pressures = self.vapor_partial_pressure_calc(fO2_best, [t])
                P_outgassed = vapor_partial_pressures.sum(axis=1).iloc[0]+fO2_best
                P_boa = P_outgassed + self.P_volatile
                partial_pressures = self.calculate_partial_pressures_fastchem_loop([self.O_abun],[t],fO2_best,[P_boa],vapor_partial_pressures,volatile_comp)
    ...
    def mass_balance_equation_fastchem(self,fO2,T, volatile_comp):
        ...
        #residuals = [self.inner_loop(O, T, fO2, P_boa, vapor_partial_pressures, volatile_comp) for O in O_abun_tries]
        
        #print(residuals)
        #for i in range(len(O_abun_tries) - 1):
            #print('try:',i)
            #if residuals[i] * residuals[i+1] < 0:
               # print(residuals[i],residuals[i+1])
               # O_low, O_high = O_abun_tries[i], O_abun_tries[i+1]
               # print(O_low, O_high)
               # break
            
        #print(O_low,O_high)
        #self.O_abun = brentq(self.inner_loop, O_low, O_high, args=(T, fO2, P_boa, vapor_partial_pressures, volatile_comp))
        ...
        def objective(logO):
            ...
            return abs(
                self.inner_loop(
                O_abun,
                T, fO2, P_boa, vapor_partial_pressures, volatile_comp
            )
        )
    ...
    def edit_fastchem_configs(self,T,P):
        ...
        tp_file = open('/data3/leoni/LavAtmos/FastChem/fastchem3/'+tp_point_path, 'w')
        tp_file.write(f'P\tT\n{P[0]:.6e}\t{T[0]:.6e}')
        tp_file.close()
